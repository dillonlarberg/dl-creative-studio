...
export interface SafeSourceUrl {
  url: URL;
  /** The single safe IP to pin the fetch to. IPv4 dotted-decimal or IPv6 hex. */
  resolvedIp: string;
  family: 4 | 6;
}
...
function ipv4MappedFromIPv6(lower: string): string | null {
 ...
 */
export async function assertSafeSourceUrl(rawUrl: string): Promise<SafeSourceUrl> {
  let url: URL;
  try {
    url = new URL(rawUrl);
  } catch {
  ...
  const first = addrs[0]!;
  return { url, resolvedIp: first.address, family: first.family as 4 | 6 };
}
