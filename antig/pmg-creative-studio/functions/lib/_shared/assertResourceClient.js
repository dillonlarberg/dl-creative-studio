...
exports.assertResourceClient = assertResourceClient;
 ...
 */
function assertResourceClient(clientSlug, resourcePath) {
    if (!resourcePath || typeof resourcePath !== 'string') {
        throw new https_1.HttpsError('permission-denied', 'Invalid resource path');
    }
    if (resourcePath.includes('..')) {
        throw new https_1.HttpsError('permission-denied', 'Resource path contains traversal');
    }
    ...
    if (resourcePath !== expectedRoot && !resourcePath.startsWith(expectedPrefix)) {
        throw new https_1.HttpsError('permission-denied', `Resource path does not belong to client '${clientSlug}'`);
    }
}
//# sourceMappingURL=assertResourceClient.js.map