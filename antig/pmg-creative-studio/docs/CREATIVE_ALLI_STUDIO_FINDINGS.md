...
- **Per-client Firestore enforcement is achievable in Phase 1 — not prototype-grade.** Because Creative Alli Studio runs inside Alli, Alli already gates client access. The `/me` endpoint (already implemented in `alli.ts`) returns the user's client list. However, the implementation is more specific than a simple Cloud Function trigger — here is the exact picture:
  - All proxy functions are plain HTTP (`onRequest`) — there is no existing `onCreate` auth trigger, and a Firebase `onCreate` trigger only receives the Firebase user object, not the Alli OIDC token, which lives in the browser's `sessionStorage` only.
  - **What works: a callable function pattern.** `auth.ts` already captures the Alli access token at login and stores it in `sessionStorage`. Immediately after login, the client calls a new Firebase callable function (`syncClientClaims`), passing the Alli token. The callable hits `getMeProxy`, gets the client list, and uses the Firebase Admin SDK to set custom claims: `setCustomUserClaims(uid, { clients: { ralph_lauren: true } })`. The client then force-refreshes the Firebase ID token (`user.getIdToken(true)`). Firestore rules enforce `request.auth.token.clients[clientSlug] == true`.
  - **The complication:** custom claims don't take effect until the client force-refreshes the ID token. There is a short window after login where Firestore writes would fail if not gated on the refresh completing.
  - **The simpler alternative:** membership documents — the callable writes `clients/{slug}/members/{uid}` for each client after login. Firestore rules use `exists()` on that path. No token refresh race condition; slightly more storage and a read on every rule evaluation.
  - **Recommendation:** callable + custom claims is the right long-term pattern (authorization lives in the token, no extra Firestore reads in rules). Membership docs are simpler to ship first. Either way, this requires a new callable function and a post-login step in the frontend — not a config change, but not a major lift. Needs a short spike to confirm the `/me` response shape includes the client list and that the token passes correctly to the callable.
